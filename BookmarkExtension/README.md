...
4. Open the extension popup, set the **API Base URL** to your server, and connect. The free-text field accepts a new server, while **Saved servers** switches between the five most recently used addresses (including local dev at `http://localhost:5080`). For a deployment with the [in-tab command palette's TLS](../Docs/deployment-ubuntu.md#tls-for-the-in-tab-command-palette-optional) set up, use the server's stable Tailscale MagicDNS name, e.g. `https://<machine>.<tailnet>.ts.net:8443`, since it stays correct even if the server's LAN IP changes. You'll be prompted for host permission for that origin — that's the only host access the extension ever requests, and only for the server you point it at. The address is saved before the permission prompt, so reopening the popup lets you finish connecting without retyping it.
