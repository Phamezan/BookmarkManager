...
describe("ServiceWorker popup toggle", () => {
  ...
  it("asks an open popup to commit rather than closing it outright", async () => {
    ...
    expect(port.postMessage).toHaveBeenCalledWith({ type: "quickBookmark/commitNow" });
    expect(query).not.toHaveBeenCalled();
  });

  it("falls back to run() when no popup port is open", async () => {
    const bookmarks = new FakeBookmarks([
      {
        id: "0",
        title: "",
    ...
    expect(worker.isPopupOpen()).toBe(false);

    await worker.handleQuickBookmark();

    // run() queries the active tab; no tab found so it warns and returns early.
