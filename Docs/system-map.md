...
- 📄 [manifest.json](BookmarkExtension/manifest.json) — Defines MV3 settings, quick-keys, scripting tab permissions, and the `bm` search omnibox keyword.
- 📄 [service-worker.ts](BookmarkExtension/src/background/service-worker.ts) — Background script orchestrating websocket loops, alarm tasks, heartbeat checks, quick-bookmark hotkeys, and address-bar search listeners.
- 📄 [bookmark-adapter.ts](BookmarkExtension/src/bookmarks/bookmark-adapter.ts) — Translates incoming server payloads into browser API calls (`chrome.bookmarks.create`, `move`, `remove`, `applyRestore` [recursive]).
...
- 📄 [Bookmarks.razor.cs](src/BookmarkManager.Client/Pages/Bookmarks.razor.cs) — Main dashboard workspace controller (drag-drop, context menus, favorites pinning, search).
- 📄 [BookmarkEditDialog.razor](src/BookmarkManager.Client/Components/BookmarkEditDialog.razor) — Form for editing bookmarks with offline tag suggestions.
- 📄 [UndoService.cs](src/BookmarkManager.Client/Services/UndoService.cs) — Stack-based global undo manager.
...
- **Extension HTML export:** `BookmarkExtension/src/backup/backup-manager.ts` exports Netscape HTML to Downloads for browser interoperability and bookmark-only recovery.
- **HTML bookmark restore:** the server `/backups` page can upload a Chromium/Netscape `.html` export and replace the contents of Brave's protected bookmark roots directly (Bookmarks bar / Other bookmarks / Mobile bookmarks), avoiding Chromium's extra `Imported bookmarks` wrapper. Restore takes a mandatory pre-restore SQLite safety snapshot, soft-deletes the old projection, and queues existing recursive Delete/Restore extension commands. Full application recovery still uses SQLite snapshots.
